...
import { setOfferActive } from "@/lib/server/repositories/supplier-product-repository";
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const updated = await setOfferActive(session.user.businessId, id, parsed.data.isActive);
    if (!updated) {
