...
import { setProductActive } from "@/lib/server/repositories/product-repository";
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const updated = await setProductActive(session.user.businessId, id, parsed.data.isActive);
    if (!updated) {
